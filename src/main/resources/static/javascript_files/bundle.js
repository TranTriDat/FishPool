  ...
  if (fishCount > fishes.length){
      $.ajax("api/fishes", {
          success: function(data){
              for (var f of data){
                fishes.push(new __WEBPACK_IMPORTED_MODULE_0__fish_js__["a" /* default */](dx , dy, radius, f , c, foods, f.headColor, f.bodyColor))
              }
          }
      });
...
"use strict";
function Fish(dx, dy, radius, id, c, foodarr, _headColor, _bodyColor) {
  let colors = [
    "#d7dde5",
    "#ea4504",
    "#d8d215",
  ...
  let headColor = _headColor;colors[Math.round(Math.random() * (colors.length - 1))];
  //let bodyColor = colors[Math.round(Math.random() * (colors.length - 1))];
  let bodyColor = _bodyColor;
